...
@final string QUERY_SET_BATCH_STATUS =
"UPDATE BatchStatus SET uuid=?, state=?, completed_time=now()";

@final string QUERY_INSERT_BATCH_STATUS =
"INSERT INTO BatchStatus(uuid, state, completed_time) VALUES(?,?,now())";

@final string QUERY_SET_BATCH_UUID = "UPDATE BatchStatus SET uuid=?";

@final string QUERY_GET_INCOMPLETE_JIRA_KEYS =
"SELECT jira_key FROM RecordStatus WHERE completed_time IS NULL";
...
@final string QUERY_BULK_UPSERT_RECORD_STATUS =
"INSERT INTO RecordStatus(jira_key, completed_time)
VALUES <ENTRIES>
...
@final string QUERY_UPDATE_RECORD_STATUS =
"UPDATE RecordStatus SET completed_time=now() WHERE jira_key=?";
